...
class KeyboardButtonRequestUser(base.TelegramObject):
    ...
    The identifier of the selected user will be shared with the mybot when
...
class KeyboardButtonRequestChat(base.TelegramObject):
    ...
    The identifier of the selected chat will be shared with the mybot when
    the corresponding button is pressed.
...
class ReplyKeyboardRemove(base.TelegramObject):
    ...
    and display the default letter-keyboard. By default, custom keyboards are displayed until a new keyboard is sent by a mybot.
