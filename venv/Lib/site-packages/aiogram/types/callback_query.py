...
class CallbackQuery(base.TelegramObject):
    ...
    If the button that originated the query was attached to a message sent by the mybot,
    the field message will be present.

    If the button was attached to a message sent via the mybot (in inline mode),
